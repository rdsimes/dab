...
let horizontalRow = (cell: ObjectRowProps) => {
 ...
 return last ? <td className='dot boxborder'></td> : 
<><td className='dot boxborder'></td>
<Line isHorizontal={true} rowIndex={cell.rowIndex} colIndex={cell.colIndex} handlers={cell.handlers}/>
</>
}
...
const VerticalRow: FC<ObjectRowProps> = (cell: ObjectRowProps) => {
  ...
  game.subscribe(cell.rowIndex, cell.colIndex??0, (owner: string) => setIsOwner(owner));

  return last? <Line isHorizontal={false} rowIndex={cell.rowIndex} colIndex={cell.colIndex} handlers={cell.handlers}/>
  :<><Line isHorizontal={false} rowIndex={cell.rowIndex} colIndex={cell.colIndex} handlers={cell.handlers} /><td className='box'><div className={owner}>box   {cell.rowIndex}   {cell.colIndex} </div></td></>
}

const ObjectRow: FC<ObjectRowProps> = ({rowIndex, colIndex, handlers}: ObjectRowProps) => 
rowIndex === ROWS - 1 ? 
...
<tr key={"v-"+rowIndex}>
{[...Array(COLUMNS)].map((x, colIndex) =>
<VerticalRow  key={"vr-"+rowIndex + "-" + colIndex} rowIndex = {rowIndex} colIndex={colIndex} handlers={handlers} />
  )}
</tr>
</>
...
const Line: FC<LineProps> = (lineProps :LineProps) => {
  ...
  return  <td className={lineClass} onClick={click}> {lineProps.isHorizontal? "vl" : "hl"} </td>
...
const Board: FC = () => {
  ...
  const handlers: GameHandlers = { SetIsPlayer1Turn,  SetPlayer1Score,  SetPlayer2Score};
 return  <div>
    <PlayerScores IsPlayer1Turn={isPlayer1Turn} Player1Score={player1Score} Player2Score={player2Score} />
    <table>
